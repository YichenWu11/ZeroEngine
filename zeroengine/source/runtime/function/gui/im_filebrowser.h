...
inline ImGui::FileBrowser::FileBrowser(ImGuiFileBrowserFlags flags) :
    width_(700), height_(450), posX_(0), posY_(0), flags_(flags), openFlag_(false), closeFlag_(false), isOpened_(false), ok_(false), posIsSet_(false), inputNameBuf_(std::make_unique<std::array<char, INPUT_NAME_BUF_SIZE>>()) {
...
inline void ImGui::FileBrowser::Display() {
        ...
        for (auto& rsc : fileRecords_) {
            ...
            if (Selectable(rsc.showName.c_str(), selected,
                ...
                const bool multiSelect =
                    (flags_ & ImGuiFileBrowserFlags_MultipleSelection) && IsWindowFocused(ImGuiFocusedFlags_RootAndChildWindows) && (GetIO().KeyCtrl || GetIO().KeyShift);
    ...
    bool shouldExit =
        Button("cancel") || closeFlag_ || ((flags_ & ImGuiFileBrowserFlags_CloseOnEsc) && IsWindowFocused(ImGuiFocusedFlags_RootAndChildWindows) && IsKeyPressed(ImGuiKey_Escape));
    if (shouldExit) {
        CloseCurrentPopup();
    }
